...
function gnews_theme_setup() {
	...
	load_theme_textdomain( 'gnews', get_template_directory() . '/languages' );
	$locale = get_locale();
	...
	add_image_size( 'gnews_810', 810, 9999, false );

	register_nav_menus( array(
		'primary' => esc_html__( 'Header Menu', 'gnews' ),
		'top_menu' => esc_html__( 'Top Navigation', 'gnews' ),
		'mobile' => esc_html__( 'Mobile Menu', 'gnews' ),
		'footer_menu' => esc_html__( 'Footer Navigation', 'gnews' ),
	) );
...
add_action( 'after_setup_theme', 'gnews_theme_setup' );
...
function gnews_import_files() {
    return array(
        array(
            'import_file_name'             => esc_html__( 'Demo 1', 'gnews' ),
            ...
            'import_notice'                => esc_html__( 'Customize this theme from Appearance/Customize', 'gnews' ),
        )
    );
}
add_filter( 'pt-ocdi/import_files', 'gnews_import_files' );

// If user agree change front page layots
function gnews_after_import_setup() {
...
add_action( 'pt-ocdi/after_import', 'gnews_after_import_setup' );
