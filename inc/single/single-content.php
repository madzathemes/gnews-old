<?php function gnews_single_bottom() {

if ( false == get_theme_mod( 't_p_read_more', false ) ) { $t_p_read_more = esc_html__("Read More:", "gnews");  } else { $t_p_read_more = get_theme_mod( 't_p_read_more' ); }
if ( false == get_theme_mod( 't_p_previous_article', false ) ) { $t_p_previous_article = esc_html__("Previous article", "gnews");  } else { $t_p_previous_article = get_theme_mod( 't_p_previous_article' ); }
if ( false == get_theme_mod( 't_p_next_article', false ) ) { $t_p_next_article = esc_html__("Next article", "gnews");  } else { $t_p_next_article = get_theme_mod( 't_p_next_article' ); }
if ( false == get_theme_mod( 't_p_may_be_intrested', false ) ) { $t_p_may_be_intrested = esc_html__("You may be interested", "gnews");  } else { $t_p_may_be_intrested = get_theme_mod( 't_p_may_be_intrested' ); }
    ...
    <?php if ( get_the_author_meta( 'description' )) { ?>
      <div class="author-info">
        <div class="author-avatar">
          <?php echo get_avatar( get_the_author_meta( 'user_email' ), apply_filters( 'gnews_author_bio_avatar_size', 70 ) ); ?>
          </div>
          <div class="author-description">

            <a href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ); ?>" rel="author"><h3><?php printf( esc_html__( '%s', 'gnews' ), get_the_author() ); ?></h3></a>
...
function gnews_single_bottom_after() {
