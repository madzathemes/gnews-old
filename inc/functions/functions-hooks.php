...
function gnews_css() {
	...
	wp_enqueue_style('gnews-style', get_stylesheet_uri());

	$custom_styles = '';
	$options = get_option("gnews_theme_options");
	...
	// Footer Colors
	 ...
	 if ( $custom_styles != '' ) {
	  $css = preg_replace("/(^[\r\n]*|[\r\n]+)[\s\t]*[\r\n]+/", "\n", $custom_styles);
		wp_add_inline_style( 'gnews-style', $css );
	}

}
add_action( 'wp_enqueue_scripts', 'gnews_css');

function gnews_header_script() {
		...
		wp_enqueue_script( 'gnews_script', get_template_directory_uri(). '/inc/js/scripts.js', array( 'jquery'), '', true );
		wp_localize_script( 'gnews_script', 'ajax_posts', array( 'ajaxurl' => admin_url( 'admin-ajax.php' ), 'noposts' => esc_html__('No older posts found', 'gnews'), ));
		...
		if (!function_exists('gnews_fonts_url')) {

		    function gnews_fonts_url() {
		      ...
		      $theme_font = "Lato:400,900,700";
		         ...
		         */
		        if ( 'off' !== _x( 'on', 'Google font: on or off', 'gnews' ) ) {
		            $font_url = add_query_arg( 'family', urlencode( ''. esc_attr($theme_font) .'' ), "//fonts.googleapis.com/css" );
		        }
		        return $font_url;
		    }
		    wp_enqueue_style( 'gnews-fonts', gnews_fonts_url(), array(), '1.0.0' );
		}

}
add_action('wp_enqueue_scripts', 'gnews_header_script');

function gnews_admin_script() {
	wp_enqueue_style('gnews-admin', get_template_directory_uri().'/inc/css/admin.css');
}
add_action('admin_enqueue_scripts', 'gnews_admin_script');


add_filter('body_class','gnews_class');
function gnews_class($classes) {
