<?php function gnews_header() {
if ( false == get_theme_mod( 't_o_followers', false ) ) { $t_o_followers = esc_html__("Followers", "gnews");  } else { $t_o_followers = get_theme_mod( 't_o_followers' ); }
if ( false == get_theme_mod( 't_p_to_search', false ) ) { $t_p_to_search = esc_html__("To search type and hit enter", "gnews");  } else { $t_p_to_search = get_theme_mod( 't_p_to_search' ); }
...
$option = get_option("gnews_theme_options");
...
<div class="header-wrap" itemscope="itemscope" itemtype="http://schema.org/WPHeader">
			...
			<div class="row">
				<div class="col-md-12">
					<div class="top-nav container-fluid">
						...
						<div class="head-logo" <?php if(!empty($option['logo_width'])) { ?>  style="width:<?php echo esc_attr($option['logo_width']); ?>px" <?php } ?>><?php gnews_logo(); ?></div>
	...
	<?php	if ( true == get_theme_mod( 'mt_menu_share', true ) ) { gnews_header_fixed();	} ?>
</div>
...
<?php } ?>
<?php } add_filter('gnews_header','gnews_header');


function gnews_top_content() { $option = get_option("gnews_theme_options"); ?>
		<div class="head-nav">
			<?php if(!empty($option['url_latest'])) { ?><a class="mt_l_latest <?php if($option['url_latest']==get_the_ID()) { ?>active<?php } ?>" href="<?php echo get_permalink(esc_html($option['url_latest'])); ?>"><?php esc_html_e( 'LATEST', 'gnews' ); ?> <span><?php esc_html_e( 'Posts', 'gnews' ); ?></span></a><?php } ?>
			<?php if(!empty($option['url_popular'])) { ?><a class="mt_l_popular <?php if($option['url_popular']==get_the_ID()) { ?>active<?php } ?>" href="<?php echo get_permalink(esc_html($option['url_popular'])); ?>"><?php esc_html_e( 'POPULAR', 'gnews' ); ?> <span><?php esc_html_e( 'Posts', 'gnews' ); ?></span></a><?php } ?>
			<?php if(!empty($option['url_hot'])) { ?><a class="mt_l_hot <?php if($option['url_hot']==get_the_ID()) { ?>active<?php } ?>" href="<?php echo get_permalink(esc_html($option['url_hot'])); ?>"><?php esc_html_e( 'HOT', 'gnews' ); ?> <span><?php esc_html_e( 'Posts', 'gnews' ); ?></span></a><?php } ?>
			<?php if(!empty($option['url_trending'])) { ?>	<a class="mt_l_trending <?php if($option['url_trending']==get_the_ID()) { ?>active<?php } ?>" href="<?php echo get_permalink(esc_html($option['url_trending'])); ?>"><?php esc_html_e( 'TRENDING', 'gnews' ); ?> <span><?php esc_html_e( 'Posts', 'gnews' ); ?></span></a><?php } ?>
		</div>

		<?php if(!empty($option['header_link_url'])) { ?>
				<div class="head-bookmark">
					<a class="mt-radius" href="<?php echo esc_url($option['header_link_url']);  ?>" <?php if(!empty($option['header_link_blank'])) { if($option['header_link_blank']=="1") {?>target="_blank" <?php }} ?>><?php echo esc_attr(get_theme_mod('gnews_header_link_name', 'Add Post')); ?></a>
				</div>
		<?php } ?>
<?php }
add_filter('gnews_top_content','gnews_top_content');
...
add_filter('gnews_logo','gnews_logo');

function gnews_logo_mobile() {
...
add_filter('gnews_logo_mobile','gnews_logo_mobile');
...
add_filter('gnews_nav','gnews_nav');

function gnews_nav_fixed() {
	wp_nav_menu( array('theme_location'=>"primary",  'menu_class' => 'fixed-menu-ul',  'echo' => true, 'depth' => 1));
}
add_filter('gnews_nav_fixed','gnews_nav_fixed');

function gnews_nav_mobile() {
	wp_nav_menu( array('theme_location'=>"mobile",  'menu_class' => 'mobile',  'echo' => true, 'depth' => 2));
}
add_filter('gnews_nav_mobile','gnews_nav_mobile');

function gnews_top_menu() {
	wp_nav_menu( array('theme_location'=>"top_menu",  'menu_class' => 'top-menu',  'echo' => true, 'depth' => 1));
}
add_filter('gnews_top_menu','gnews_top_menu');

function gnews_socials() { ?>
	<ul class="social"> <?php
			$option = get_option("gnews_theme_options");
...
} add_filter('gnews_socials','gnews_socials');

function gnews_header_fixed() {
	if (is_single()) {
		...
		if ( false == get_theme_mod( 't_p_share_on_facebook', false ) ) { $t_p_share_on_facebook = esc_html__("Share on Facebook", "gnews");  } else { $t_p_share_on_facebook = get_theme_mod( 't_p_share_on_facebook' ); }
	 	if ( false == get_theme_mod( 't_p_share_on_twitter', false ) ) { $t_p_share_on_twitter = esc_html__("Tweet on Twitter", "gnews");  } else { $t_p_share_on_twitter = get_theme_mod( 't_p_share_on_twitter' ); }
		?>
		<?php $option = get_option("gnews_theme_options"); ?>
				<div class="fixed-top">
	...
	<?php } ?>
<?php } add_filter('gnews_header_fixed','gnews_header_fixed'); ?>
